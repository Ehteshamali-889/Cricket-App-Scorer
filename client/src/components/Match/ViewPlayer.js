...
export default function ViewPlayer(props) {
  ...
  useEffect(()=>{
    const fetchData=()=>{
      fetch('http://localhost:8080/api/users/allplayer/'+first)
    ...
    fetchData()
    const fetchData2=()=>{
      fetch('http://localhost:8080/api/users/allplayer/'+second)
      .then(response=>response.json())
      .then(json=>{
          console.log(json);
          setSecUsers(json);
  ...
  return (
    <Box sx={{ width: '100%' }}>
      ...
      <TabPanel value={value} index={0}>
        <FirstTeam users={users} />
      </TabPanel>
      <TabPanel value={value} index={1}>
        <SecondTeam secusers={secusers} />
      </TabPanel>
